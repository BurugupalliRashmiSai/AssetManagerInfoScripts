...
#     seller_info["sellername2"]=seller_info["sellername2"].astype(str)
...
#     print("seller last and first name record number match",seller_last_first.record_number.nunique())
    merge_data_lastname["Scoring"]=merge_data_lastname.apply(lambda x:  
    1 if x["managerfirstname"]==x["sellerfirstname"] and x["managermiddlename"]==x["sellermidname"] else
    (2 if  x["managerfirstname"]==x["sellerfirstname"] else 3),axis=1)
    return merge_data_lastname
...
#     print("owner last and first name record number match",owner_last_first.record_number.nunique())
    merge_data_lastname["Scoring"]=merge_data_lastname.apply(lambda x: 
                                                         1 if x["managerfirstname"]==x["owner1firstnamemi"] 
                                                          and x["managermiddlename"]==x["owner1midname"]
                                                         else 
                                    (2 if x["managerfirstname"]==x["owner1firstnamemi"] else 3),axis=1)
    return merge_data_lastname
...
def get_first_lastname_owner_data_v2(manager_data_notna,owner2_info):
    ...
    merge_data_lastname["Scoring"]=3
    owner_last_first=merge_data_lastname[merge_data_lastname["managerfirstname"]==merge_data_lastname["owner2firstnamemi"]]
    merge_data_lastname["Scoring"]=merge_data_lastname.apply(lambda x: 
                                                             1 if x["managerfirstname"]==x["owner2firstnamemi"] 
                                                              and x["managermiddlename"]==x["owner2midname"]
                                                             else (2 if 
                                                             x["managerfirstname"]==x["owner2firstnamemi"] else 
                                                             3),axis=1)
    ...
    print(merge_data_lastname["Scoring"].value_counts())
...
dir_path = r'/scratch/user/rashmi_1996/Data'
for path in os.listdir(dir_path):
    print("Running for next file")
...
#         print(pd.merge(manager_data_notna,mortage_data,left_on="managername",right_on="sellername2").shape)
        ...
        try:
            ...
            pass
        gc.collect()
        final_output_file.to_csv("/scratch/user/rashmi_1996/final_output_file.csv")
...
# import numpy as np
# no_of_rows = 50000
# for k,g in df.groupby(np.arange(len(final_output_file))//no_of_rows):
...
# final_output_file=pd.read_csv(dir_path+"/final_output_file.csv")
# final_output_file.head()
print("Number of unique manager ids:",final_output_file["obs_manager"].nunique())
print("Number of unique managers with total match:",
final_output_file[final_output_file["Scoring"]==1]["obs_manager"].nunique())
print("Number of unique manager ids with only last name matches:",
final_output_file[final_output_file["Scoring"]==3]["obs_manager"].nunique())
print("Number of unique manager ids with first and last name matches:",
final_output_file[final_output_file["Scoring"]==2]["obs_manager"].nunique())
